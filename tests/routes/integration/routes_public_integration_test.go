...
// TestRoutesPublicIntegration Test Routes Charts for Private Inputs.
func TestRoutesPublicIntegration(t *testing.T) {
	t.Parallel()

	// Path to the helm chart we will test
	helmChartPath, err := filepath.Abs("../../../charts/routes")

	releaseName := "helm-routes-public" + strings.ToLower(random.UniqueId())
	...
	values := routes.Routes{
		Global: routes.Global{
			...
			Upstream: routes.Upstream{
				Name: "grafana.grafana.svc.cluster.local",
				Port: 80,
			},
	...
	k8s.WaitUntilSecretAvailable(t,
		...
		releaseName+"-tls", 300, 5*time.Second)
	...
	for _, host := range externalEndpoints {
	...
	for _, host := range internalEndpoints {
		http_helper.HttpGetWithRetryWithCustomValidation(
			t,
			fmt.Sprintf("https://%s", host),
			&tls.Config{
		...
		for _, ip := range ips {
			if ipv4 := ip.To4(); ipv4 != nil {
				require.False(t, subnet.Contains(ipv4))
			}
		}
	}
}
